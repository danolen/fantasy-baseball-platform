...
select {{ dbt_utils.star(source('razzball', 'projections_preseason_pitching')) }},
...
from {{ source('razzball', 'projections_preseason_pitching') }}
where concat(regexp_extract("$path", 'year=([0-9]{4})', 1),
    ...
    regexp_extract("$path", 'day=([0-9]{1,2})', 1)) = (select max(concat(regexp_extract("$path", 'year=([0-9]{4})', 1),
                                                        regexp_extract("$path", 'month=([0-9]{1,2})', 1),
                                                        regexp_extract("$path", 'day=([0-9]{1,2})', 1))) from {{ source('razzball', 'projections_preseason_pitching') }})