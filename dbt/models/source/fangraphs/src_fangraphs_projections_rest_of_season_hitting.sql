...
select {{ dbt_utils.star(source('fangraphs', 'projections_rest_of_season_hitting')) }},
...
from {{ source('fangraphs', 'projections_rest_of_season_hitting') }}
where concat(regexp_extract("$path", 'year=([0-9]{4})', 1),
    ...
    regexp_extract("$path", 'day=([0-9]{1,2})', 1)) = (select max(concat(regexp_extract("$path", 'year=([0-9]{4})', 1),
                                                        regexp_extract("$path", 'month=([0-9]{1,2})', 1),
                                                        regexp_extract("$path", 'day=([0-9]{1,2})', 1))) from {{ source('fangraphs', 'projections_rest_of_season_hitting') }})
