...
-- Raw NFBC overall overview snapshots. History is retained; latest selection
-- happens in staging (#182). Snapshot fields come from Athena "$path".
select {{ dbt_utils.star(source('nfbc', 'in_season_overall_overview')) }},
...
from {{ source('nfbc', 'in_season_overall_overview') }}
