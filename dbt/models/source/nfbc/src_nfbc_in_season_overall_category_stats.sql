...
-- Raw NFBC overall category-stat snapshots. History is retained; latest
-- selection happens in staging (#182). Snapshot fields come from Athena "$path".
select {{ dbt_utils.star(source('nfbc', 'in_season_overall_category_stats')) }},
...
from {{ source('nfbc', 'in_season_overall_category_stats') }}
