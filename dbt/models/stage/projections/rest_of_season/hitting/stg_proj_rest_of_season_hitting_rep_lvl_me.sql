...
with base as (
    select *,
        ...
        case when position in ('1B','2B','3B','SS') and row_number() over (partition by position order by sgp desc) <= 15 then 'Y'
            when position = 'OF' and row_number() over (partition by position order by sgp desc) <= 15*5 then 'Y'
            when position = 'C'  and row_number() over (partition by position order by sgp desc) <= 15*2 then 'Y'
            else 'N' end as include_in_pool
    from {{ ref('stg_proj_rest_of_season_hitting_sgp_me') }}
),

remaining_pos_group as (
    select *,
        case when pos_group in ('MI','CI') and row_number() over (partition by pos_group order by sgp desc) <= 15 then 'Y'
