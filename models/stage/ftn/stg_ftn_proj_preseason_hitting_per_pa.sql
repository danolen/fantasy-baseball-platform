...
select ids.id,
    ...
    cast(proj.bb_hitter as double)/cast(proj.pa as double) as bb,
    cast(proj.hbp_hitter as double)/cast(proj.pa as double) as hbp
from {{ ref('src_ftn_projections_preseason') }} proj
inner join {{ ref('stg_mpd_player_id_map') }} ids
    on proj.nfbcid = ids.id
where (proj.position not like '%SP%'
    and proj.position not like '%RP%')
    or (proj.nfbcid = '9810'
    and proj.position like '%UT%')